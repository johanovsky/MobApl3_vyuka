...
function addCityToFavorites() {
    //najdeme div s mestem
    const cityDiv = document.getElementById("city");
    let cityName;
    //pokud mame div
    if(cityDiv !== null) {
        //vytahnu z nej nadpis h3
        const h3 = cityDiv.querySelector("h3");
        //pokud mame h3 nadpis
        if(h3 !== null) {
            //vytahnu mesto
            cityName = h3.innerText;
        }
    }

    //test jestli mam mesto
    if(cityName !== undefined) {
        addFavorite(cityName)
        .then(result => {
            if(result) alert("Mesto " + cityName + " pridano do DB");
            else alert("Mesto " + cityName + " uz je mezi oblibenymi");
        })
        .catch(err => {
            alert("Chyba pri ukladani");
        });
    } else {
        alert("Mesto nezadano");
    }
}