...
function addToFavorites() {
    //najdeme div s uziv. mestem
    const cityDiv = document.getElementById("city");
    let cityName = null;
    if(cityDiv !== null) {
        //kdyz mame div, tak v nem najdeme h3
        const cityH3 = cityDiv.querySelector("h3");
        //kdyz mam h3, stahnu z ni jmeno mesta
        if(cityH3 !== null) {
            cityName = cityH3.innerText;
        }
    }

    //jestlize mam jmeno mesta
    if(cityName !== null) {
        //budeme pridavat mesto do localDB
        addFavorite(cityName)
        .then(result => {
            if(result) alert("Mesto " + cityName + " bylo pridano mezi oblibene");
            else alert(cityName + " uz je mezi oblibenymi");
        })
        .catch(err => {
            alert("Chyba prace s localDB");
        });
    } else {
        //nemame mesto -> nic nepridavame
