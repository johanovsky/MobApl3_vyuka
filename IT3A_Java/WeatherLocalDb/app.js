...
if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
            console.log("Chyba pri ziskavani polohy: ", error);
        }
    );
} else {
    console.log("Geolocation neni v prohlizeci podporovana");
}
    ...
    if(city != "") {
        ...
        .then(data => {
            ...
            if(data.cod === 200) {
                ...
                if(zCache === true) {
                    div.innerHTML += "<p>Pozor, data jsou z cache</p>";
                }
...
function addToFavorites() {
    ...
    //promenne pro nazev mesta
    let cityName = null;
    if(cityDiv !== null) {
        const cityH3 = cityDiv.querySelector("h3");
        if(cityH3 !== null) {
            cityName = cityH3.innerText;
        }
    }

    //kdyz mame mesto
    if(cityName !== null) {
        //povedlo se nam vytahnout jmeno mesta
        addFavorite(cityName)
        .then(result => {
            if(result) alert("Mesto " + cityName + " pridano mezi oblibene");
            else alert(cityName + " jiz je mezi oblibenymi");
        })
        .catch(err => {
            alert("Chyba pri praci s DB");
        });
    } else {
        //vubec nemame mesto
        alert("Mesto nebylo zadano");
    }
}
